...
void myfilewg::addUploadFiles()
{
    ...
    for(int i = 0; i < list.size(); ++i)
    {
        ...
        {
            QMessageBox::warning(this, "添加失败", "上传的文件是否已经在上传列表中!!");
        }
        else if(res == -3)
        {
            cout << "打开文件失败";
...
QStringList myfilewg::getCountStatus(QByteArray json)
{
    ...
    //由ByteArray对象构造一个QJsonDocument对象，用于我们的读写操作
...
void myfilewg::refreshFiles(myfilewg::Display cmd)
{
    ...
    //127.0.0.1:8080/myfiles&cmd=count  //获取用户文件个数
    ...
    connect(reply, &QNetworkReply::finished, [=]()
    {
        ...
        if( list.at(0) == "111" )
        {
            QMessageBox::warning(this, "帐号异常", "请重新登录！");
        ...
        cout<< "userFileCount = " << m_userFilesCount;
...
void myfilewg::getFileJsonInfo(QByteArray data)
{
    ...
    */
    ...
    if(error.error == QJsonParseError::NoError) //没有出错
    {
        ...
        if(doc.isObject())
        {
            //QJsonObiect json对象，描述json数据中{}括起来部分
            QJsonObject obj = doc.object(); //取得最外面的这个大对象

            //QJsonArray json数组，描述json数据中[]括起来部分
            QJsonArray array = obj.value("files").toArray();

            int size = array.size();    //数组个数,即文件个数
            cout << "size = " << size;
    ...
    for(int i = 0; i < m_fileList.size(); ++i)
    {
        if(m_fileList.at(i)->item == item)
        {
            ...
            */
            ...
            if(res == -1)
            {
                QMessageBox::warning(this, "任务已存在", "任务已经在任务下载列表中");
            }
...
void myfilewg::downloadFilesAction()
{
    ...
    if(p->isDownload()) //当前时间没有任务在下载才能下载。单任务
    {
        return;
    }

    //看是否是共享文件下载任务，不是才能往下执行，如果是，中断程序
    ...
    //当有可用数据时，reply就会发出readyRead()信号，我们这是就可以将可用的数据保存下来
...
void myfilewg::checkTaskList()
{
    ...
    {
        //下载文件处理，取出上传任务列表的队首任务，上传完后，再取下一个任务
        downloadFilesAction();
    });
...
void myfilewg::rightMenu(const QPoint &pos)
{
    ...
    else //点图标
    {
        ...
        if(item->text() == "上传文件") //如果是上传文件图标，没有右击菜单
        {
            return;
        }
...
{
    if(info->shareStatus == 1)  //已经分享，不能再分享了
    {
    ...
    //127.0.0.1:8080/dealfile?cmd=share
    ...
    */
    ...
    connect(reply, &QNetworkReply::finished, [=]()
    {
        ...
        {
            QMessageBox::warning(this, "帐号异常", QString("请重新登录！"));
...
void myfilewg::delFile(FileInfo *info)
{
    ...
    //127.0.0.1:8080/dealfile?cmd=del
