    ...
    */
    ...
    if (error.error == QJsonParseError::NoError) // 没有出错
    {
        ...
        if(doc.isObject())
        {
            ...
            int size = array.size();    //json数组个数

            for(int i = 0; i < size; ++i)
            {
                QJsonObject tmp = array[i].toObject();  //去第i个对象
                /*
                    文件信息
                    struct FileInfo
                    {
        ...
        if(m_shareFileList.at(i)->item == item)
        {
            ...
            */
            ...
            else if(res == -2)  //打开文件失败
            {
                ...
                m_cm.writeRecord(login->getUser(), m_shareFileList.at(i)->filename, "010"); //记录下载文件失败
            }
            break;
        }
    }
...
void sharelist::downloadFilesAction()
{
    ...
    //当有可用数据时，reply就会发出readyread信号，我们这时就可以将可用的数据保存下来
